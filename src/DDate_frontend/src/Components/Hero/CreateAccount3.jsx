...
const CreateAccount3 = () => {
  ...
  const handlesmokingSelect = (value) => {
    setSelectedsmoking(value);
  };

  const handledrinkSelect = (value) => {
    setSelecteddrink(value);
  };

  const handlehobbiesSelect = (value) => {
      ...
      <h2 className="text-3xl md:text-4xl font-bold mb-4 text-white md:text-black text-center">Allow us to know you</h2>
        ...
        <form className="w-full max-w-lg rounded-lg p-6 shadow-md md:bg-transparent md:shadow-none" onSubmit={e => e.preventDefault()}>
            ...
            <fieldset className="mb-2">
              <legend className="block text-lg font-semibold mb-1 text-white md:text-black">Smoking</legend>
              <div className="flex flex-wrap gap-2 md:gap-2 mb-4 py-2 px-2 rounded-3xl">
                {['Regular', 'Sometimes', 'Never'].map((smoking) => (
                  <button
                    key={smoking}
                    type="button"
                    onClick={() => handlesmokingSelect(smoking)}
            ...
            <fieldset className="mb-2">
              <legend className="block text-lg font-semibold mb-1 text-white md:text-black">Alcohol/Drink</legend>
            ...
            <fieldset className="mb-2">
              <legend className="block text-lg font-semibold mb-2 text-white md:text-black">Hobbies (select any 2)</legend>
              <div className="flex flex-wrap gap-2 md:gap-2 mb-2 py-2 px-2 rounded-3xl">
                {['Reading', 'Dancing', 'Singing','Cooking','Photography','Comics','Gardening'].map((hobbies) => (
                  <button
                    key={hobbies}
                    type="button"
                    onClick={() => handlehobbiesSelect(hobbies)}
                    className={`inline-block px-3 py-2 rounded-full text-sm focus:outline-none transition duration-300 ${
            ...
            <fieldset className="mb-2">
              <legend className="block text-lg font-semibold mb-1 text-white md:text-black">Sports (select any 2)</legend>
              <div className="flex flex-wrap gap-2 md:gap-2 mb-2 py-2 px-2 rounded-3xl">
                {['Cricket', 'Football', 'Basketball', 'Tennis','Chess','Badminton','Boxing','Gym','Yoga','Volleyball','Cricket','Carrom','Golf','Table-Tennis','Weightlifting','Gymnastics','Polo','Rugby','Cycling','Wrestling','Swimming'].map((sports) => (
                  <button
                    key={sports}
                    type="button"
                    onClick={() => handlesportsSelect(sports)}
                    className={`inline-block px-3 py-2 rounded-full text-sm focus:outline-none transition duration-300 ${
