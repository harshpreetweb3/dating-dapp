...
function Swipe() {
  ...
  const onPointerUp = useCallback(() => {
    if (Math.abs(moveX) > window.innerWidth / 2) {
      ...
      setTimeout(() => setCards(prevCards => prevCards.slice(1)), window.innerWidth);
    } else {
      ...
      <div className="p-4 sm:ml-64">
        <div className="container flex justify-center px-4">
          <div className="max-w-xs md:max-w-md lg:max-w-lg xl:max-w-xl bg-white rounded-lg shadow-lg overflow-hidden">
            ...
            <div className="frame">
              {/* Swipe buttons */}
              <button onClick={handleLeftSwipe}>Swipe Left</button>
              <button onClick={handleRightSwipe}>Swipe Right</button>

              {/* Cards */}
              {cards.map((card, index) => (
                <div key={index} className="card" style={{ backgroundImage: `url(${card.img})` }}>
                  <div className="is-like">LIKE</div>
                  <div className="bottom">
                    <div className="title">
                      <span>{card.name}</span>
                      <span><b>$</b>{card.price}</span>
                    </div>
                    <div className="info">
              ...
              <div class="icons flex px-4">
                <svg id="hate" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 128.07 127.89" onClick={handleLeftSwipe}>
                ...
                <svg id="like" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 128.06 127.99" onClick={handleRightSwipe}>
